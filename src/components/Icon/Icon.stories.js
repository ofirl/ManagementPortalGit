            ...
            chapters: [
                {
                    sections: [
                        {
                            sectionFn: () => {
                                // const bsPrefix = text('bsPrefix', 'fe');

                                let typeTypes = allTypesString.split(',');
                                const type = select("Type", typeTypes, 'search');

                                const inline = boolean("Inline", false);

                                return (<Icon type={type} inline={inline} />);
                            },
                            options: {
                                showPropTables: true
                            }
            ...
            chapters: [
                {
                    sections: [
                        {
                            sectionFn: () => (<Icon type="search" onClick={action('onClick')} />),
                            options: {
                                showPropTables: true
                            }
                        }
                    ...
                    sections: [
                        {
                            ...
                            sectionFn: () => {
                                return (
                                    <div>
                                        {
                                            allTypesString.split(',').map((t) => (<Icon key={t} type={t} className="p-2" />))
                                        }
                                    </div>
                                );
                            },
